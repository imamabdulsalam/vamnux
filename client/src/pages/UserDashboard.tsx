...
export default function UserDashboard() {
  ...
  const renderContent = () => {
    ...
    if (activeTab === "wallet") return <section className="user-panel"><div className="user-panel-heading"><div><span>WALLET ACTIVITY</span><h2>Your account balance</h2><p>Wallet movements are shown only when recorded in your immutable account ledger.</p></div><WalletCards size={23} /></div><div className="user-wallet-feature"><div><p>Available balance</p><strong>{money(data.wallet.availableBalance, data.wallet.currency)}</strong><span className="user-status">{data.wallet.status}</span></div><div><h3>Wallet funding is not active yet</h3><p>Funding and wallet payments will remain unavailable until VAMNUX connects an approved payment provider. No balance has been simulated.</p></div></div>{data.walletEntries.length === 0 ? <EmptyState title="No wallet entries" detail="Your account has no recorded wallet activity. Any future credit, debit, purchase, refund, or adjustment will appear with its actual status." /> : <div className="user-wallet-list">{data.walletEntries.map((entry) => <article key={entry.id}><div className={`user-entry-icon ${entry.direction}`}><WalletCards size={16} /></div><div><strong>{statusLabel(entry.entryType)}</strong><p>{entry.reference} · {new Date(entry.createdAt).toLocaleString()}</p></div><div><b className={entry.direction === "credit" ? "credit" : "debit"}>{entry.direction === "credit" ? "+" : "−"}{money(entry.amount, entry.currency)}</b><small>{entry.status}</small></div></article>)}</div>}</section>;
