...
type ManualCatalogForm = {
  name: string;
  category: "gift_card" | "subscription" | "software" | "ai_tool" | "game_key" | "steam" | "steam_top_up" | "telegram_stars";
  description: string;
...
export function ManualCatalogManager() {
  ...
  const [isReviewingProduct, setIsReviewingProduct] = useState(false);
  ...
  const submitProduct = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!isReviewingProduct) {
      setIsReviewingProduct(true);
      return;
    }
  ...
  return (
    <section className="mt-7 overflow-hidden rounded-xl border border-[#286dff]/30 bg-white shadow-sm">
      <div className="flex flex-col justify-between gap-4 border-b border-slate-200 bg-[#f7f9ff] px-6 py-5 sm:flex-row sm:items-end"><div><p className="text-xs font-bold tracking-[.13em] text-[#286dff]">AUTHORISED CATALOG MANAGEMENT</p><h2 className="mt-2 font-['Barlow_Condensed'] text-3xl font-bold uppercase">Add verified digital inventory</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Create only products covered by an approved supplier or direct commercial agreement. Every item is linked to an active source record and stays in draft unless you deliberately set it live. This tool never creates a supplier order, payment, wallet credit, or delivery record.</p></div><button type="button" onClick={() => { const setup = document.getElementById("manual-product-setup"); setup?.scrollIntoView({ behavior: "smooth", block: "start" }); setup?.querySelector<HTMLInputElement>("input")?.focus(); }} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#b8ff43] px-5 py-3 text-xs font-extrabold uppercase tracking-[.1em] text-[#10121a] shadow-[0_8px_20px_rgba(184,255,67,.28)] transition hover:bg-[#10121a] hover:text-white"><Plus size={15} />Add manual product</button></div>
      ...
      <div className="grid gap-0 lg:grid-cols-[1.05fr_.95fr]">
        <form onSubmit={submitProduct} className="space-y-5 p-6" id="manual-product-setup">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#b8ff43]/70 bg-[#10121a] p-4 text-white"><div><p className="text-xs font-bold tracking-[.13em] text-[#b8ff43]">GUIDED PRODUCT SETUP</p><h3 className="mt-1 font-['Barlow_Condensed'] text-2xl font-bold uppercase">{isReviewingProduct ? "Review your listing" : "Create a manual product"}</h3><p className="mt-1 text-xs leading-5 text-slate-300">{isReviewingProduct ? "Confirm the customer-facing details before VAMNUX records this authorised listing." : "Complete each section, then review the listing before it can be saved."}</p></div><span className="rounded-full border border-white/20 px-3 py-1 text-[10px] font-bold uppercase tracking-[.1em] text-[#b8ff43]">{isReviewingProduct ? "Step 5 of 5" : "Steps 1–4"}</span></div>
          {isReviewingProduct ? <div className="space-y-4"><div className="rounded-xl border border-[#286dff]/30 bg-[#f7f9ff] p-4"><p className="text-xs font-bold tracking-[.1em] text-[#286dff]">CUSTOMER-FACING PREVIEW</p><div className="mt-3 flex gap-4">{form.imageUrl ? <img src={form.imageUrl} alt="Manual product preview" className="h-20 w-20 rounded-lg border border-slate-200 object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-lg bg-[#10121a] text-xs font-bold text-[#b8ff43]">VAMNUX</div>}<div><h4 className="text-lg font-bold text-[#10121a]">{form.name}</h4><p className="mt-1 text-sm text-slate-600">{form.description}</p><p className="mt-2 text-sm font-bold text-[#10121a]">USD {Number(form.basePrice).toFixed(2)} · {form.regionLabel || "Region not specified"}</p></div></div></div><dl className="grid gap-3 text-sm sm:grid-cols-2"><div className="rounded-lg border border-slate-200 p-3"><dt className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-500">Product details</dt><dd className="mt-1 font-semibold text-[#10121a]">{form.category.replaceAll("_", " ")} · {form.platform || "Platform not specified"} · {form.productType || "Type not specified"}</dd></div><div className="rounded-lg border border-slate-200 p-3"><dt className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-500">Approved source</dt><dd className="mt-1 font-semibold text-[#10121a]">{activeSources.find((source) => source.id === Number(form.catalogSourceId))?.displayName || "No active source selected"}</dd></div><div className="rounded-lg border border-slate-200 p-3"><dt className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-500">Fulfilment & delivery</dt><dd className="mt-1 font-semibold text-[#10121a]">{form.deliveryType.replaceAll("_", " ")} · {form.deliveryMinimumMinutes || "—"}–{form.deliveryMaximumMinutes || "—"} minutes</dd></div><div className="rounded-lg border border-slate-200 p-3"><dt className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-500">Initial state</dt><dd className="mt-1 font-semibold text-[#10121a]">{form.status} · Recipient email {form.recipientEmailRequired ? "required" : "not required"}</dd></div></dl><div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><strong>Review safeguard.</strong> Saving records product metadata only. It does not create an order, call a supplier, move wallet funds, collect payment, or promise delivery.</div><div className="flex flex-wrap gap-3"><button type="button" onClick={() => setIsReviewingProduct(false)} className="inline-flex items-center gap-2 rounded-full border border-slate-300 px-5 py-3 text-xs font-extrabold uppercase tracking-[.1em] text-slate-700 transition hover:border-[#286dff]">Edit details</button><button type="submit" disabled={createProduct.isPending || activeSources.length === 0} className="inline-flex items-center gap-2 rounded-full bg-[#b8ff43] px-5 py-3 text-xs font-extrabold uppercase tracking-[.1em] text-[#10121a] transition hover:bg-[#10121a] hover:text-white disabled:cursor-not-allowed disabled:opacity-60"><CheckCircle2 size={15} />{createProduct.isPending ? "Saving authorised listing…" : "Confirm & save listing"}</button></div></div> : <><section className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-bold tracking-[.1em] text-[#286dff]">STEP 1 · PRODUCT IDENTITY</p><div className="mt-3 grid gap-4 sm:grid-cols-2"><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Product name<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="e.g. Free Fire 530 Diamonds" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none transition focus:border-[#286dff] focus:ring-2 focus:ring-[#286dff]/15" /></label><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Category<select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value as ManualCatalogForm["category"] })} className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]"><option value="gift_card">Gift card</option><option value="subscription">Subscription</option><option value="software">Software</option><option value="ai_tool">AI tool</option><option value="game_key">Game key</option><option value="steam">Games</option><option value="steam_top_up">Steam Top-Up</option><option value="telegram_stars">Telegram Stars</option></select></label><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Platform or game <span className="normal-case tracking-normal">(optional)</span><input value={form.platform} onChange={(event) => setForm({ ...form, platform: event.target.value })} placeholder="e.g. PUBG Mobile, Netflix, Spotify" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]" /></label><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Product type / subtype <span className="normal-case tracking-normal">(optional)</span><input value={form.productType} onChange={(event) => setForm({ ...form, productType: event.target.value })} placeholder="e.g. Top-up, monthly plan, 50 USD code" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]" /></label></div><label className="mt-4 block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Customer-facing description<textarea required value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="State the verified product, denomination or plan, region, and delivery terms without unsupported promises." className="mt-2 min-h-24 w-full rounded-lg border border-slate-300 bg-white p-3 text-sm text-[#10121a] outline-none transition focus:border-[#286dff] focus:ring-2 focus:ring-[#286dff]/15" /></label><label className="mt-4 block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Image URL <span className="normal-case tracking-normal">(optional)</span><input type="url" value={form.imageUrl} onChange={(event) => setForm({ ...form, imageUrl: event.target.value })} placeholder="https://…/product-image.png" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]" /><span className="mt-1 block normal-case font-normal tracking-normal text-slate-500">Use a verified HTTPS image URL you are authorised to display.</span></label></section><section className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-bold tracking-[.1em] text-[#286dff]">STEP 2 · SOURCE, PRICE & REGION</p><div className="mt-3 grid gap-4 sm:grid-cols-3"><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500 sm:col-span-2">Authorised source<select required value={form.catalogSourceId} onChange={(event) => setForm({ ...form, catalogSourceId: event.target.value })} disabled={sourceQuery.isLoading || activeSources.length === 0} className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none disabled:bg-slate-100"><option value="">{sourceQuery.isLoading ? "Loading sources…" : activeSources.length === 0 ? "Add an active source above" : "Select an active source"}</option>{activeSources.map((source) => <option key={source.id} value={source.id}>{source.displayName} · {source.sourceType.replaceAll("_", " ")}</option>)}</select><span className="mt-1 block normal-case font-normal tracking-normal text-slate-500">Only an active approved source can be attached to a new listing.</span></label><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">USD base price<input required inputMode="decimal" value={form.basePrice} onChange={(event) => setForm({ ...form, basePrice: event.target.value })} placeholder="0.00" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none transition focus:border-[#286dff] focus:ring-2 focus:ring-[#286dff]/15" /></label></div><label className="mt-4 block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Region label <span className="normal-case tracking-normal">(optional)</span><input value={form.regionLabel} onChange={(event) => setForm({ ...form, regionLabel: event.target.value })} placeholder="Global, Nigeria, United States, or a supported region" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]" /></label></section><section className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-bold tracking-[.1em] text-[#286dff]">STEP 3 · FULFILMENT & DELIVERY</p><div className="mt-3 grid gap-4 sm:grid-cols-3"><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Fulfilment type<select value={form.deliveryType} onChange={(event) => setForm({ ...form, deliveryType: event.target.value as ManualCatalogForm["deliveryType"] })} className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]"><option value="digital_code">Digital code</option><option value="activation_link">Activation link</option><option value="manual_processing">Manual processing</option><option value="account_access">Account access</option></select></label><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Minimum delivery <span className="normal-case">(minutes)</span><input inputMode="numeric" value={form.deliveryMinimumMinutes} onChange={(event) => setForm({ ...form, deliveryMinimumMinutes: event.target.value.replace(/[^0-9]/g, "") })} placeholder="e.g. 5" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]" /></label><label className="block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Maximum delivery <span className="normal-case">(minutes)</span><input inputMode="numeric" value={form.deliveryMaximumMinutes} onChange={(event) => setForm({ ...form, deliveryMaximumMinutes: event.target.value.replace(/[^0-9]/g, "") })} placeholder="e.g. 60" className="mt-2 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]" /></label></div><p className="mt-3 text-xs leading-5 text-slate-500">Use a realistic delivery range. Leave both values blank only where the verified source has no timing guidance.</p></section><section className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-bold tracking-[.1em] text-[#286dff]">STEP 4 · CUSTOMER REQUIREMENTS & STATUS</p><label className="mt-3 block text-xs font-bold uppercase tracking-[.1em] text-slate-500">Customer requirements <span className="normal-case">(optional)</span><textarea value={form.customerRequirements} maxLength={1000} onChange={(event) => setForm({ ...form, customerRequirements: event.target.value })} placeholder="For example: account ID, player ID, server, region, or other verified instructions." className="mt-2 min-h-20 w-full rounded-lg border border-slate-300 bg-white p-3 text-sm text-[#10121a] outline-none focus:border-[#286dff]" /></label><div className="mt-4 flex flex-col gap-3 rounded-lg bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between"><label className="flex items-center gap-3 text-sm text-slate-700"><input type="checkbox" checked={form.recipientEmailRequired} onChange={(event) => setForm({ ...form, recipientEmailRequired: event.target.checked })} className="h-4 w-4 accent-[#286dff]" />Require a recipient email at draft order</label><label className="flex items-center gap-2 text-sm text-slate-700">Initial state<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ManualCatalogForm["status"] })} className="rounded border border-slate-300 bg-white px-2 py-1.5 text-sm"><option value="draft">Draft (recommended)</option><option value="active">Active</option><option value="paused">Paused</option></select></label></div></section><button type="submit" disabled={activeSources.length === 0} className="inline-flex items-center gap-2 rounded-full bg-[#286dff] px-5 py-3 text-xs font-extrabold uppercase tracking-[.1em] text-white transition hover:bg-[#10121a] disabled:cursor-not-allowed disabled:opacity-60"><ArrowRight size={15} />Review listing before save</button></>}
        </form>
