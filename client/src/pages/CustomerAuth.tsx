...
function RegistrationField({ label, children, note }: { label: string; children: ReactNode; note?: string }) {
  return (
    <label className="grid gap-1 text-[8px] font-extrabold uppercase tracking-[.07em] text-slate-300">
      <span>{label}</span>
      {children}
      {note && <small className="text-[8px] normal-case tracking-normal text-[#b8ff43]">{note}</small>}
    </label>
  );
}
