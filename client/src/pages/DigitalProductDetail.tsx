...
import { ArrowLeft, ArrowRight, CircleDollarSign, Gift, Globe2, Heart, Laptop, Search, Send, ShieldCheck, ShoppingBag, Sparkles, Tv, X } from "lucide-react";
...
export default function DigitalProductDetail() {
  ...
  const addToCart = () => {
    ...
    toast.success(`${product.product} added to your cart`, { description: "VAMNUX products are prepared for wallet-only purchase. No direct payment or supplier fulfilment is available." });
  };
  const saveDraft = () => {
  ...
  const isSaved = customerDashboard.data?.savedProducts.some((savedProduct) => savedProduct.id === product.id) ?? false;
  const saveProduct = () => {
    ...
    toggleSavedProduct.mutate({ productId: product.id });
  };
  return <main className="game-family-page">
    ...
    <section className="family-services" aria-label={`${product.product} details`}><div className="family-services-heading"><div><p className="detail-eyebrow">Product details</p><h2>READY TO<br /><em>REVIEW.</em></h2></div><p>Review the shown price, regional information, delivery format, and any stated delivery estimate before saving your draft selection.</p></div><div className="family-service-list"><article className={`family-service-row tone-${product.tone}`}><div className="family-service-name"><h3>{product.product}</h3><p>{product.description}</p><div className="market-tags"><span>{product.region}</span><span>{product.delivery}</span>{product.deliveryEstimate && <span>Estimate: {product.deliveryEstimate}</span>}</div></div><div className="family-service-price"><strong>{formatPrice(product.price)}</strong><small>{product.priceNote} · USD base</small></div><div className="family-service-actions"><button type="button" onClick={saveProduct} disabled={toggleSavedProduct.isPending} className={isSaved ? "saved" : ""}>{isSaved ? "Saved" : "Save"} <Heart size={15} fill={isSaved ? "currentColor" : "none"} /></button><button type="button" onClick={addToCart}>Add to cart <ArrowRight size={17} /></button></div></article></div></section>
