...
export default function DigitalProductDetail() {
  ...
  return <main className="game-family-page">
    ...
    <section className="family-services" aria-label={`${product.product} options`}><div className="family-services-heading"><div><p className="detail-eyebrow">Choose an option</p><h2>SELECT<br /><em>YOUR ITEM.</em></h2></div><p>Every option is an active VAMNUX product listing. Select a denomination, review its exact region and requirement, then continue to protected wallet checkout.</p></div>{selectedItem && <div className="family-selection-layout"><div className="family-denomination-area"><div className="family-requirement-bar"><span>What’s needed for this selection</span>{selectedItem.inputRequirements.filter((field) => field.required).length ? selectedItem.inputRequirements.filter((field) => field.required).map((field) => <b key={field.key}>{field.label}</b>) : <b>No account field required</b>}</div><div className="family-denomination-grid">{familyItems.map((item) => <button key={item.id} type="button" onClick={() => setSelectedServiceId(item.id)} className={item.id === selectedItem.id ? "family-denomination-card selected" : "family-denomination-card"}><span>{item.product}</span><strong>{formatPrice(item.price)}</strong><small>{item.region} · {item.delivery}</small></button>)}</div></div><aside className="family-selection-summary"><p className="detail-eyebrow">Your selection</p><h3>{selectedItem.product}</h3><p>{selectedItem.name}</p><div className="selection-required-fields">{selectedItem.inputRequirements.filter((field) => field.required).map((field) => { const key = createFulfillmentFieldKey(selectedItem.id, field.key); return <label key={key}><span>{field.label} *</span><input type={field.type === "email" ? "email" : "text"} value={fulfillmentDetails[key] ?? ""} onChange={(event) => setFulfillmentDetails((current) => ({ ...current, [key]: event.target.value }))} placeholder={field.helperText || field.label} /></label>; })}</div><div className="selection-price"><span>VAMNUX price</span><strong>{formatPrice(selectedItem.price)}</strong><small>{selectedItem.priceNote}</small></div><div className="digital-selection-actions"><button type="button" onClick={saveProduct} disabled={toggleSavedProduct.isPending} className={isSaved ? "saved" : ""}>{isSaved ? "Saved" : "Save"} <Heart size={15} fill={isSaved ? "currentColor" : "none"} /></button><button onClick={() => addToCart(selectedItem)} aria-label={`Checkout ${selectedItem.product} with wallet`} title="Checkout with wallet"><ShoppingCart size={19} /><span>Checkout with wallet</span></button></div><small className="selection-note">Continue to the cart to complete the protected wallet checkout. Automatic ordering and delivery remain inactive.</small></aside></div>}</section>
