...
const runner = readFileSync("/home/ubuntu/gamesdrop_playstation_import_2026-08-25.mjs", "utf8");

describe("GamesDrop PlayStation-only import safeguards", () => {
  it("locks supplier retrieval and normalized rows to Games plus exact PlayStation platform evidence", () => {
    expect(runner).toContain('const SUPPLIER_CATEGORY = "Games"');
    expect(runner).toContain('const TARGET_PLATFORM = "playstation"');
  ...
  it("uses exact source identities and rejects source or slug conflicts before transactional insert", () => {
  ...
  it("preserves cost/currency and creates traceable unmapped snapshots and append-only audits", () => {
    ...
    expect(runner).toContain('catalog_import.gamesdrop_playstation_supplier_offer_created');
    expect(runner).toContain('pricingConfigurationUntouched: true');
  });

  it("does not write prices, orders, wallets, routing, fulfilment, or existing products", () => {
