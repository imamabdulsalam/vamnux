...
describe("Super Admin authorization", () => {
  ...
  it("rejects an authenticated customer before finance, policy, monitoring, reseller, settings, or global-search data can be accessed", async () => {
    ...
    await expect(caller.admin.bulkUpdateProductStorefrontVisibility({ productIds: [1, 2], storefrontStatus: "hidden" })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.admin.bulkArchiveAdminManagedCatalogProducts({ productIds: [1, 2] })).rejects.toMatchObject({ code: "FORBIDDEN" });
