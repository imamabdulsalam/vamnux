...
const runner = readFileSync("/home/ubuntu/gamesdrop_battlenet_import_2026-08-25.mjs", "utf8");

describe("GamesDrop Battle.net-only import safeguards", () => {
  it("locks supplier retrieval and normalized rows to Games plus exact Nintendo platform evidence", () => {
    expect(runner).toContain('const SUPPLIER_CATEGORY = "Games"');
    expect(runner).toContain('const TARGET_PLATFORM = "battlenet"');
  ...
  it("preserves cost/currency and creates traceable unmapped snapshots and append-only audits", () => {
    ...
    expect(runner).toContain('catalog_import.gamesdrop_nintendo_supplier_offer_created');
