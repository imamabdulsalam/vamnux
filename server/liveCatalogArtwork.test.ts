...
describe("live catalog artwork fallbacks", () => {
  it("uses an existing authorized supplier family image when a Mobile Legends or PUBG supplier row has no artwork", () => {
    expect(toLiveCatalogProduct({ ...baseCatalogRow, name: "Mobile Legends Global — 86 Diamonds", imageUrl: null }, 0).image).toBe("/media/mobile-legends_da301a0e.webp");
    expect(toLiveCatalogProduct({ ...baseCatalogRow, name: "PUBG Mobile — 60 UC", imageUrl: null }, 1).image).toBe("/media/pubg-mobile_66e3513a.webp");
  });
