...
export const appRouter = router({
  ...
  marketplace: router({
    catalog: publicProcedure.input(z.object({
      ...
      scope: z.enum(["primary", "all"]).optional(),
    }).optional()).query(({ input }) => listActiveCatalogProducts(input)),
    catalogProduct: publicProcedure.input(z.object({ slug: z.string().trim().min(1).max(180) })).query(({ input }) => getActiveCatalogProductDetail(input.slug)),
    catalogGameFamily: publicProcedure.input(z.object({ familyName: z.string().trim().min(1).max(255) })).query(({ input }) => getActiveCatalogGameFamily(input.familyName)),
