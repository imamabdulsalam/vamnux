...
export const appRouter = router({
  ...
  admin: router({
    ...
    getTopUpControlDashboard: adminProcedure.query(() => getTopUpControlDashboard()),
    listTopUpWebhookMonitor: adminProcedure.input(z.object({ search: z.string().trim().max(160).optional(), status: z.enum(["received", "verified", "credited", "duplicate", "rejected", "error", "reconciled"]).optional(), provider: z.string().trim().max(120).optional(), currency: z.string().trim().length(3).optional(), start: z.coerce.date().optional(), end: z.coerce.date().optional(), offset: z.number().int().min(0).default(0), limit: z.number().int().min(1).max(100).default(50) }).refine((input) => !input.start || !input.end || input.start <= input.end, { message: "The start date must be before the end date" }).optional()).query(({ input }) => listTopUpWebhookMonitor(input)),
    listTopUpTransactions: adminProcedure.input(z.object({ search: z.string().trim().max(160).optional(), status: z.enum(["initialized", "pending", "settled", "failed", "expired", "cancelled", "reversed"]).optional(), provider: z.string().trim().max(120).optional(), currency: z.string().trim().length(3).optional(), start: z.coerce.date().optional(), end: z.coerce.date().optional(), offset: z.number().int().min(0).default(0), limit: z.number().int().min(1).max(100).default(50) }).refine((input) => !input.start || !input.end || input.start <= input.end, { message: "The start date must be before the end date" }).optional()).query(({ input }) => listTopUpTransactions(input)),
    listTopUpReconciliationCases: adminProcedure.input(z.object({ search: z.string().trim().max(160).optional(), status: z.enum(["open", "resolved"]).optional(), category: z.enum(["missing_wallet_credit", "duplicate_event", "duplicate_reference", "invalid_signature", "amount_currency_mismatch", "provider_failed", "provider_pending", "refunded_or_reversed"]).optional(), provider: z.string().trim().max(120).optional(), start: z.coerce.date().optional(), end: z.coerce.date().optional(), offset: z.number().int().min(0).default(0), limit: z.number().int().min(1).max(100).default(50) }).refine((input) => !input.start || !input.end || input.start <= input.end, { message: "The start date must be before the end date" }).optional()).query(({ input }) => listTopUpReconciliationCases(input)),
    listUserWalletTimeline: adminProcedure.input(z.object({ search: z.string().trim().max(160).optional(), status: z.enum(["pending", "completed", "reversed", "failed"]).optional(), category: z.enum(["funding", "purchase", "refund", "adjustment", "reward"]).optional(), currency: z.string().trim().length(3).optional(), start: z.coerce.date().optional(), end: z.coerce.date().optional(), offset: z.number().int().min(0).default(0), limit: z.number().int().min(1).max(100).default(50) }).refine((input) => !input.start || !input.end || input.start <= input.end, { message: "The start date must be before the end date" }).optional()).query(({ input }) => listUserWalletTimeline(input)),
    listTopUpControlAudit: adminProcedure.input(z.object({ search: z.string().trim().max(160).optional(), start: z.coerce.date().optional(), end: z.coerce.date().optional(), offset: z.number().int().min(0).default(0), limit: z.number().int().min(1).max(100).default(50) }).refine((input) => !input.start || !input.end || input.start <= input.end, { message: "The start date must be before the end date" }).optional()).query(({ input }) => listTopUpControlAudit(input)),
    resolveTopUpReconciliationCase: adminProcedure.input(z.object({ caseId: z.number().int().positive(), resolutionNote: z.string().trim().min(3).max(1000) })).mutation(({ ctx, input }) => resolveTopUpReconciliationCase({ adminUserId: ctx.user.id, ...input })),
    createManualWalletAdjustment: adminProcedure.input(z.object({ email: z.string().trim().email().max(320), direction: z.enum(["credit", "debit"]), amount: z.number().positive().max(1_000_000), currency: z.string().trim().length(3), reason: z.string().trim().min(3).max(1000), confirmed: z.literal(true) })).mutation(({ ctx, input }) => createManualWalletAdjustment({ adminUserId: ctx.user.id, email: input.email, direction: input.direction, amount: input.amount, currency: input.currency, reason: input.reason })),
    reverseWalletLedgerEntry: adminProcedure.input(z.object({ walletEntryId: z.number().int().positive(), reference: z.string().trim().min(3).max(120), reason: z.string().trim().min(3).max(1000), confirmation: z.literal("CONFIRM WALLET REVERSAL") })).mutation(({ ctx, input }) => reverseWalletLedgerEntry({ adminUserId: ctx.user.id, walletEntryId: input.walletEntryId, reference: input.reference, reason: input.reason })),
    ...
    listSimplePricingTargets: adminProcedure.query(() => listSimplePricingTargets()),
    ...
    updateScopedMarketplaceMarkup: adminProcedure.input(z.object({
      scope: z.enum(["global", "category", "product", "supplier"]),
      category: z.enum(["top_up", "gift_card", "game_key", "subscription", "ai_tool", "software", "steam", "steam_top_up", "telegram_stars"]).nullable().optional(),
      productId: z.number().int().positive().nullable().optional(),
      supplierKey: z.string().trim().min(1).max(80).nullable().optional(),
      markupPercent: z.number().min(-100).max(500),
    })).mutation(({ ctx, input }) => updateScopedMarketplaceMarkup({ ...input, adminUserId: ctx.user.id })),
    ...
    listAdminProductOperations: adminProcedure.input(z.object({ limit: z.number().int().min(1).max(10_000).default(10_000), offset: z.number().int().min(0).default(0), search: z.string().trim().min(2).max(100).optional() }).optional()).query(({ input }) => listAdminProductOperations(input?.limit, input?.offset, input?.search)),
