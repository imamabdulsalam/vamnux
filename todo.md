...
- [x] Inspect the current GamesDrop secure integration, existing GamesDrop-linked products, source identifiers, approved Games/Gift Cards/Top-up category taxonomy, supplier-derived subcategory metadata, pricing configuration, and preservation baseline before importing anything.
- [x] Retrieve the current GamesDrop catalogue through the secure server-side integration and classify only verified Games, Gift Cards, and Game Top Ups records (including Games platform and Top-up Direct Top Up/Activation Codes subcategories) without deleting, recategorizing, or duplicating existing VAMNUX products.
- [x] Insert only verified new GamesDrop records using unique supplier source identifiers, preserve source cost/currency/attributes, retain pricing-policy compatibility, organize supplier-derived subcategories, and leave customer prices, mappings, routing, fulfilment, orders, wallets, transactions, and storefront behavior unchanged.
- [x] Verify duplicate prevention, category/subcategory organization, supplier traceability, pricing isolation, tests/build, visual review, and preservation counts; publish and push the verified GamesDrop reconciliation report.
- [x] Obtain a GamesDrop-authoritative Games and Game Top Ups subcategory taxonomy or per-offer subcategory field before any import, rather than deriving customer taxonomy from product names.
- [x] Prepare only GamesDrop-supported or owner-provided VAMNUX Games and Top-up subcategories, with an explicit Unclassified/Admin Review path for offers lacking authoritative subcategory evidence.
- [x] Extend the prepared Games taxonomy with the complete GamesDrop supplier-provided platform codes returned by the full read-only catalog reconciliation, without assigning any product from its title.
- [x] Analyze existing VAMNUX catalog and Supplier Offer records by primary category and verified subcategory evidence without deleting, duplicating, importing, repricing, or modifying supplier source data.
- [x] Produce the required pre-import GamesDrop reconciliation report identifying category preparation, safe classifications, Admin Review cases, existing sources, genuinely new sources, and potential duplicates; stop before any supplier-product import.
- [x] Implement and test a transaction-batched, insert-only GamesDrop importer that checks exact source SKU/offer identity and never updates existing product or Supplier Offer records.
