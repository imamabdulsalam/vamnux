...
- [ ] Assess and plan the owner-requested migration of VAMNUX to existing Namecheap hosting and domain, including Node runtime, database, environment secrets, DNS, Paystack TEST webhooks, backups, rollback, and explicit cutover approval before any deployment.
...
- [ ] Implement the owner-approved staged Supabase Auth, Resend transactional-email, owner-controlled storage, and Product Tracking scheduler migration with verified identity linking, tests, cPanel readiness checks, final synchronization, and explicit public cutover approval while preserving VAMNUX data and the current deployment as rollback.
- [ ] Guide the owner through Resend sending-domain verification for VAMNUX using Namecheap DNS, then configure the existing Resend API key securely in cPanel and test only approved transactional email behavior before any public cutover.
...
- [ ] Implement secure server-side Resend transactional email support for the approved VAMNUX external-authentication migration, with non-sending tests and one owner-approved cPanel test before public cutover.
