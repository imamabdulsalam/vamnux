...
- [x] Provide the owner with a safe Namecheap custom-domain deployment sequence that keeps VAMNUX on managed hosting and Paystack in TEST mode until an explicit later LIVE-mode approval.
- [x] Assess and plan the owner-requested migration of VAMNUX to existing Namecheap hosting and domain, including Node runtime, database, environment secrets, DNS, Paystack TEST webhooks, backups, rollback, and explicit cutover approval before any deployment.
- [x] Inspect and resolve only the production dependency conflict while retaining Node.js 22 and Vite 7; determine whether the JSX location plugin is required, update package/configuration and lockfile safely, and validate type checking and production build without changing VAMNUX functionality or data.
- [x] Re-verify the npm production dependency resolution while retaining Node.js 22 and Vite 7; confirm the obsolete JSX location plugin is absent or safely compatible, run npm install, type-check and production build, then checkpoint and push only validated package files without altering VAMNUX application data or functionality.
- [x] Configure cPanel deployment to install the required Vite 7 build-time devDependencies before `npm run build` while retaining a `NODE_ENV=production` runtime; validate clean npm install, type-check, build, checkpoint, and GitHub push without changing VAMNUX functionality or data.
- [x] Make normal cPanel npm installation reliably include Vite 7 and all VAMNUX build tooling by aligning the committed package manifest and npm lock/configuration; verify a clean production-mode npm install creates `node_modules/vite`, type-checks, builds, checkpoints, and pushes without changing application functionality or data.
- [x] Diagnose and resolve the cPanel Node 22 tsx/esbuild binary-version mismatch; validate one clean production-mode npm install, type-check, production build with `dist/index.js`, and startup readiness, then checkpoint and push without changing VAMNUX functionality or data.
- [x] Reduce VAMNUX production build peak memory for Namecheap shared cPanel/CloudLinux without changing dependencies, Node.js 22, Vite 7, application functionality, data, or production startup; validate clean npm install, check, build output, startup, checkpoint, and GitHub push.
- [x] Prepare and validate a prebuilt Node.js 22 VAMNUX runtime artifact for external cPanel deployment that does not compile Vite on the host; document exact upload contents, secure runtime environment requirements, database/API connectivity prerequisites, startup steps, checkpoint, and GitHub push without changing application functionality or data.
- [x] Prepare the new Namecheap VAMNUX database user credential in a safe connection-string format before entering it in cPanel, without exposing the password or changing the original VAMNUX database.
- [x] Prepare a controlled external-hosting replacement plan for VAMNUX Manus OAuth and managed services, including data-preserving user-account compatibility, secure service alternatives, rollback, and owner approval before any cPanel restart or public DNS cutover.
- [x] Supersede the rejected Supabase Auth plan with the owner-approved Namecheap MySQL-native customer authentication and Resend email direction; storage and Product Tracking scheduler replacement remain separate future work.
- [x] Replace the previously proposed Supabase direction with a staged Namecheap MySQL-native authentication foundation, verified Resend email integration, existing-user enrollment, administrator-transition block, and OAuth rollback while preserving VAMNUX account IDs.
- [x] Guide the owner through Resend sending-domain verification for VAMNUX using Namecheap DNS; secure cPanel key configuration and an approved private test remain pending below.
- [x] Verify the owner-controlled Resend sending domain `send.vamnux.com` through Namecheap DNS without affecting VAMNUX website routing.
- [x] Implement secure server-side Resend transactional email support for the approved VAMNUX external-authentication migration with deterministic non-sending tests; one owner-approved cPanel test remains pending before public cutover.
- [x] Correct the malformed Resend SPF host `resend.send` to the Resend-issued `rsend.send`, then verify the sending subdomain without modifying VAMNUX website records.
- [x] Add a configuration-gated Namecheap MySQL-native customer account foundation with scrypt password hashes, opaque hashed sessions, one-time verification/reset tokens, five-failure lockout, Resend email client safeguards, existing-email enrollment, and an explicitly blocked native-admin transition; preserve OAuth fallback and existing numeric customer IDs.
- [x] Wire the customer account page to the native sign-in, registration, verified enrollment, and password-reset procedures only when `VAMNUX_NATIVE_AUTH_ENABLED=true`; retain the existing managed OAuth handoff while the live Manus deployment remains the rollback path.
- [x] Type-check, run focused non-sending native-auth/Resend tests, and build the complete production bundle after the gated native-account work.
- [x] Define and review one canonical idempotent MySQL schema script for the empty native-auth tables and pending-registration records before applying it to the Namecheap database copy; do not apply generated drift migrations 0044/0045 blindly.
